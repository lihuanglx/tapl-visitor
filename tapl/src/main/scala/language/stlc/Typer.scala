...
trait Typer[A[-R, E, -F] <: Term[R, E, F], B[-X, Y] <: Type[X, Y]]
  ...
  def tmApp(e1: Exp2[A, Exp[B]], e2: Exp2[A, Exp[B]]): (List[(String, Exp[B])]) => Exp[B] =
    c => {
      val t1 = convertStlc(apply(e1)(c)).getOrElse(sys.error("Conversion failed"))
      val t2 = apply(e2)(c)
