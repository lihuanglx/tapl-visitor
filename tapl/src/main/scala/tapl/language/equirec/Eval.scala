...
import tapl.language.equirec.Term._

trait Eval[A[-R, E, -F] <: Term[R, E, F], V] extends Term[Exp2[A, V], Exp2[A, V], V] with typed.Eval[A, V]
...
trait Subst[A[-R, E, -F] <: Term[R, E, F], V] extends Transform[A, V] with typed.Subst[A, V]
