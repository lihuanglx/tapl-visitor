...
case class CError[A[-R, E, -F] <: Alg[R, E, F], V]() extends TExp[A, V] {
  override def apply[E](alg: A[Exp[({type lam[-X, Y] = A[X, Y, V]})#lam], E, V]): E = alg.TmError()
}

case class CTry[A[-R, E, -F] <: Alg[R, E, F], V](e1: TExp[A, V], e2: TExp[A, V]) extends TExp[A, V] {
  override def apply[E](alg: A[Exp[({type lam[-X, Y] = A[X, Y, V]})#lam], E, V]): E = alg.TmTry(e1, e2)
}

trait Factory extends bot.Alg.Factory with typedbool.Alg.Factory {
...
trait TFactory extends bot.TAlg.Factory with typedbool.TAlg.Factory
