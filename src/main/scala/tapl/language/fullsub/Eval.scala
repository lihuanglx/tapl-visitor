...
trait Eval[A[-R, E, -F] <: Alg[R, E, F], V] extends Alg[E3[A, V], E3[A, V], V] with simple.Eval[A, V]
...
trait IsVal[A[-R, E, -F], V] extends Query[E3[A, V], Boolean, V] with simple.IsVal[A, V]
