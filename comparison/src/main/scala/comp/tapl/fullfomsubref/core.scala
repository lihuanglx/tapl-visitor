...
object Typer {
  ...
  // subtyping
  private def promote(ctx: Context, t: Ty): Ty = t match {
    case TyVar(i, _) => ctx.getBinding(i) match {
  ...
  def subtype(ctx: Context, ty1: Ty, ty2: Ty): Boolean = {
    ...
    (tyS, tyT) match {
      ...
      case (TyAll(tyX1, tyS1, tyS2), TyAll(_, tyT1, tyT2)) =>
        subtype(ctx, tyS1, tyT1) && subtype(ctx, tyT1, tyS1) && subtype(ctx.addBinding(tyX1, TyVarBind(tyT1)), tyS2, tyT2)
      case (TyAbs(tyX1, knKS1, tyS2), TyAbs(_, knKT1, tyT2)) =>
