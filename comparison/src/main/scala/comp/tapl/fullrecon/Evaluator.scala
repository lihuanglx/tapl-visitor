...
object Typer {
  ...
  def recon(ctx: Context, nextUVar: UVarGenerator, t: Term): (Ty, UVarGenerator, Constr) = t match {
    ...
    case TmLet(x, t1, t2) =>
      if (!isVal(ctx, t1)) {
        val (tyT1, nextuvar1, constr1) = recon(ctx, nextUVar, t1)
        val ctx1 = ctx.addBinding(x, VarBind(tyT1))
        val (tyT2, nextuvar2, constr2) = recon(ctx1, nextuvar1, t2)
    ...
    case TmIf(t1, t2, t3) =>
      ...
      val newconstr = List((tyT1, TyBool), (tyT2, tyT3))
  ...
  def applySub(ctr: IdConstr, tyT: Ty): Ty = {
    ctr.reverse.foldLeft(tyT)((tyS, ctr) => ctr match {
      case (TyId(tyX), tyC2) => substInTy(tyX, tyC2, tyS)
    })
  }
