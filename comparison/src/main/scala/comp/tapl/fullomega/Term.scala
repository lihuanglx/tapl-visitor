...
case class TyVarBind(k: Kind) extends Binding
...
object Syntax {
  ...
  def bindingShift(d: Int, bind: Binding): Binding = bind match {
...
object PrettyPrinter {
  ...
  def pknKind(outer: Boolean, ctx: Context, k: Kind): Document = k match {
    case knK => pknArrowKind(outer, ctx, knK)
  }

  def pknArrowKind(outer: Boolean, ctx: Context, k: Kind): Document = k match {
  ...
  def pknAKind(outer: Boolean, ctx: Context, k: Kind): Document = k match {
    case KnStar => "*"
  ...
  def prokn(ctx: Context, knk: Kind): Document = knk match {
    case KnStar => empty
  ...
  def ptyType(outer: Boolean, ctx: Context, ty: Ty): Document = ty match {
    ...
    case TyAll(tyX, knK1, tyT2) =>
      ...
      g2("All " :: tyX1 :: prokn(ctx, knK1) :: "." :/: ptyType(outer, ctx1, tyT2))
  ...
  def ptyAppType(outer: Boolean, ctx: Context, tyT: Ty): Document = tyT match {
  ...
  def ptyAType(outer: Boolean, ctx: Context, tyT: Ty): Document = tyT match {
    ...
    case TySome(tyX, knK1, tyT2) =>
      val (ctx1, tyX1) = ctx.pickFreshName(tyX)
      g2("{Some " :: tyX1 :: prokn(ctx, knK1) :: "," :/: ptyType(false, ctx1, tyT2) :: "}")
    case tyT =>
  ...
  def ptmTerm(outer: Boolean, ctx: Context, t: Term): Document = t match {
    ...
    case TmTAbs(x, knK, t) =>
      val (ctx1, x1) = ctx.pickFreshName(x)
      val abs = g0("lambda" :/: x1 :: prokn(ctx, knK) :: ".")
      val body = ptmTerm(outer, ctx1, t)
      g2(abs :/: body)
    case TmUnPack(tyX, x, t1, t2) =>
      val (ctx1, tyX1) = ctx.pickFreshName(tyX)
      val (ctx2, x1) = ctx1.pickFreshName(x)
      g2("let {" :: tyX1 :: ", " :: x :: "} =" :/: ptmTerm(false, ctx, t1) :/: "in " :: ptmTerm(outer, ctx2, t2))
  ...
  def ptmATerm(outer: Boolean, ctx: Context, t: Term): Document = t match {
    ...
    case TmPack(tyT1, t2, tyT3) =>
      g2("{*" :: ptyType(false, ctx, tyT1) :: "," :/: ptmTerm(false, ctx, t2) :: "}" :/: "as " :: ptyType(outer, ctx, tyT3))
    case t =>
      "(" :: ptmTerm(outer, ctx, t) :: ")"
  }
