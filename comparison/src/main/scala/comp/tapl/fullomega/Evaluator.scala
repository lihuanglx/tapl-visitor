...
case class Store(l: List[Term] = List()) {
  ...
  def extend(v: Term): (Int, Store) = (l.length, Store(l :+ v))

  def lookup(i: Int): Term = l(i)

  def update(n: Int, v: Term): Store = Store(l.updated(n, v))

  def shift(i: Int): Store = Store(l.map(termShift(i, _)))
}
