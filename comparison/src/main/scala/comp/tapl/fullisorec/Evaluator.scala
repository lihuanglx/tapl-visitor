...
object Util {
  ...
  def isVal(ctx: Context, t: Term): Boolean = t match {
    ...
    case TmUnit => true
    case TmApp(TmFold(_), v) => isVal(ctx, v)
    case _ => false
  }
}
...
object Evaluator {
  ...
  private def eval1(ctx: Context, t: Term): Term = t match {
    ...
    case TmApp(TmUnfold(tyS), TmApp(TmFold(tyT), v)) if isVal(ctx, v) =>
      v
    case TmApp(TmFold(tyS), t2) =>
      TmApp(TmFold(tyS), eval1(ctx, t2))
    case TmApp(TmUnfold(tyS), t2) =>
      TmApp(TmUnfold(tyS), eval1(ctx, t2))
...
object Typer {
  ...
  def typeof(ctx: Context, t: Term): Ty = t match {
    ...
    case TmFold(tyS) => simplifyTy(ctx, tyS) match {
      case TyRec(_, tyT) =>
